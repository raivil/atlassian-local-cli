.PHONY: setup build clean test test-cov wiki-export wiki-update wiki-create jira-create jira-link-epic jira-get jira-my-tasks jira-transition jira-update jira-me jira-open jira-search jira-comment jira-comments jira-link jira-unlink jira-link-types jira-worklog jira-sprints jira-sprint-add jira-sprint-issues jira-clone jira-delete jira-epics jira-epic-issues
...
jira-comment: ## Add a comment. Usage: make jira-comment ISSUE=<key> BODY="<text>" | BODY_FILE=<file>
	@if [ -z "$(ISSUE)" ]; then echo "Error: ISSUE is required."; exit 1; fi
	uv run atlassian-local-cli jira-comment $(ISSUE) $(if $(BODY),--body "$(BODY)") $(if $(BODY_FILE),--body-file $(BODY_FILE))

jira-comments: ## List comments. Usage: make jira-comments ISSUE=<key>
	@if [ -z "$(ISSUE)" ]; then echo "Error: ISSUE is required."; exit 1; fi
	uv run atlassian-local-cli jira-comments $(ISSUE) $(if $(JSON),--json)
...
jira-link: ## Link two issues. Usage: make jira-link FROM=<key> TO=<key> TYPE=Blocks [COMMENT="..."]
	...
	uv run atlassian-local-cli jira-link $(FROM) $(TO) --type "$(TYPE)" $(if $(COMMENT),--comment "$(COMMENT)")

jira-unlink: ## Remove a link. Usage: make jira-unlink LINK_ID=<id>
	@if [ -z "$(LINK_ID)" ]; then echo "Error: LINK_ID is required."; exit 1; fi
	uv run atlassian-local-cli jira-unlink $(LINK_ID)

jira-link-types: ## List link types
	uv run atlassian-local-cli jira-link-types $(if $(JSON),--json)
...
jira-worklog: ## Log work. Usage: make jira-worklog ISSUE=<key> TIME="2h 30m" [COMMENT="..."]
	...
	uv run atlassian-local-cli jira-worklog $(ISSUE) --time "$(TIME)" $(if $(COMMENT),--comment "$(COMMENT)") $(if $(STARTED),--started "$(STARTED)")
...
jira-sprints: ## List sprints. Usage: make jira-sprints BOARD=<id> [STATE=active]
	...
	uv run atlassian-local-cli jira-sprints --board $(BOARD) $(if $(STATE),--state $(STATE)) $(if $(JSON),--json)
...
jira-sprint-add: ## Add issues to sprint. Usage: make jira-sprint-add SPRINT=<id> ISSUES="A-1 A-2"
	...
	uv run atlassian-local-cli jira-sprint-add $(SPRINT) $(ISSUES)
...
jira-sprint-issues: ## List sprint issues. Usage: make jira-sprint-issues SPRINT=<id>
	...
	uv run atlassian-local-cli jira-sprint-issues $(SPRINT) $(if $(LIMIT),--limit $(LIMIT)) $(if $(JSON),--json)

jira-clone: ## Clone an issue. Usage: make jira-clone ISSUE=<key> [SUMMARY="..."] [REPLACE="old:new ..."]
	@if [ -z "$(ISSUE)" ]; then echo "Error: ISSUE is required."; exit 1; fi
	uv run atlassian-local-cli jira-clone $(ISSUE) $(if $(SUMMARY),--summary "$(SUMMARY)") $(foreach r,$(REPLACE),--replace "$(r)")
...
jira-delete: ## Delete an issue. Usage: make jira-delete ISSUE=<key> YES=1 [CASCADE=1]
	...
	uv run atlassian-local-cli jira-delete $(ISSUE) --yes $(if $(CASCADE),--cascade)

jira-epics: ## List epics. Usage: make jira-epics [PROJECT=PROJ] [STATUS=open]
	uv run atlassian-local-cli jira-epics $(if $(PROJECT),--project $(PROJECT)) $(if $(STATUS),--status $(STATUS)) $(if $(LIMIT),--limit $(LIMIT)) $(if $(JSON),--json)

jira-epic-issues: ## List epic children. Usage: make jira-epic-issues EPIC=<key>
	@if [ -z "$(EPIC)" ]; then echo "Error: EPIC is required."; exit 1; fi
	uv run atlassian-local-cli jira-epic-issues $(EPIC) $(if $(LIMIT),--limit $(LIMIT)) $(if $(JSON),--json)
...
jira-update: ## Update a Jira issue. Usage: make jira-update ISSUE=<key> [SUMMARY="<text>"] [DESCRIPTION="<text>"] [DESC_FILE=<file>] [PRIORITY=High] [ASSIGNEE=user] [TYPE=Task] [EPIC=PROJ-100] [LABELS="a b"] [ADD_LABELS="a b"] [REMOVE_LABELS="a b"] [FIELDS="key1=val1 key2=val2"]
	@if [ -z "$(ISSUE)" ]; then echo "Error: ISSUE is required."; exit 1; fi
	uv run atlassian-local-cli jira-update $(ISSUE) \
