.PHONY: setup build clean wiki-export wiki-update wiki-create jira-get jira-my-tasks jira-transition
...
wiki-export: ## Export a wiki page. Usage: make wiki-export PAGE=<page_id> [OUTPUT=<file.md>]
	...
	uv run python main.py wiki-export $(PAGE) $(if $(OUTPUT),-o $(OUTPUT))
...
wiki-update: ## Update a wiki page. Usage: make wiki-update PAGE=<page_id> INPUT=<file.md>
	...
	uv run python main.py wiki-update $(PAGE) $(INPUT)
