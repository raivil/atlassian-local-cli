...
### Fixed
- `create_confluence()` now picks its auth mode the way `create_jira()` has since v2.8.0. It chose basic vs Bearer purely on whether `WIKI_USERNAME` was set, with no notion of Cloud: a Cloud context that omitted the key sent `Authorization: Bearer`, which the site rejects with a 403, surfacing as a bare `HTTPError` that named nothing. A `*.atlassian.net` host now selects basic auth on its own and, when `WIKI_USERNAME` is missing, fails with a message naming the key and the file to put it in. Verified against a live Cloud site: Bearer returns 403 there, basic returns 200.
  - A Server/DC username still selects basic auth, unlike the Jira rule which ignores `JIRA_USERNAME` entirely. Confluence has always chosen its auth that way, so existing Server configs keep working; the URL check only adds the Cloud case. The three tests covering those Server paths passed before the change, which is what makes them regression guards.
  - New `WIKI_AUTH=basic|bearer` overrides the detection, mirroring `JIRA_AUTH` — for Cloud on a custom domain, or to opt an `atlassian.net` host out.
